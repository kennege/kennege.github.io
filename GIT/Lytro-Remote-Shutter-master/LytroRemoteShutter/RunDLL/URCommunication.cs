...
namespace RunDLL
{
    ...
    public class URCommunication
    {
        ...
        public static int Communicate(string calib)
        {
            ...
            {
                int j = 0;
                foreach (string str in pose) // for each pose
            {           
                Console.WriteLine("Moving to" + pose[j]);
                Console.WriteLine(Program.matlab.Execute(String.Format(@"moverobot(s,{0})", pose[j]))); // move robot to next pose
                if (j==0)
                    {
                ...
                Console.WriteLine("Triggering Lytro");  
                Program.lytronetclient.TakePicture();    // take picture       
                System.Threading.Thread.Sleep(1500);
                j = j + 1;
            }
                int count = j;
